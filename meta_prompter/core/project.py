...
class Project(BaseModel):
    ...
    generation_jobs: Dict[str, GenerationJobConfig] = Field(
        default_factory=dict, description="Generation job configurations"
    )
    ...
    def stage_documents(self, source: str) -> int:
        ...
        """
        source_dir = (
            self.get_scraped_dir() if source == "scraped" else self.get_cleaned_dir()
        )
    ...
    def get_staged_documents(self) -> List[Path]:
        ...
        return sorted(
            [f for f in staged_dir.iterdir() if f.is_file() and f.suffix == ".md"]
        )
...
<content>
    ...
    def from_yaml(cls, yaml_path: Path) -> "Project":
        ...
        with open(yaml_path, "r") as f:
            ...
            if (
                "generation_jobs" not in config_dict
                or config_dict["generation_jobs"] is None
            ):
    ...
    def to_yaml(self, yaml_path: Path) -> None:
        ...
        config_dict["scrape_job"]["seed_urls"] = [
            str(url) for url in self.scrape_job.seed_urls
        ]
    ...
    def create(
        ...
        directories = ["scraped", "cleaned", "staged", "meta_prompts"]
    ...
    def generate_context(self, job, staged_docs: List[Path]) -> str:
        ...
        encoding = tiktoken.get_encoding(
            "cl100k_base"
        )  # Base encoding for Gemini models
        ...
        prompt = prompt_template.format(
            topic=self.generation_jobs[job].topic, content=input_doc
        )
